...
void context_mask(BufferPtr input, const Buffer& mask) {
    ...
    bool is_bert = mask.shape()[3] > input->shape()[3];
    ...
    parallel_for(N, [&](int tid) {
        ...
        for (int i = 0; i < dim2 * dim3; i++) {
            ...
            auto m = is_bert? mask.dataWithOffset(i / dim3 * mask.shape()[2] + (i % dim3)) : mask.dataWithOffset(b * dim2 * dim3 + i);
...
void processSoftmaxMask(const SoftmaxParams& params) {
    ...
    bool is_bert = params.mask.value().get().shape()[3] > input->shape()[3];
    ...
    parallel_for(batch_size * num_heads, [&](int idx) {
        ...
        parallel_for(q_length, [&](int j) {
            ...
            size_t mask_offset = is_bert? j * params.mask.value().get().shape()[2] : (m * q_length + j) * k_length;
